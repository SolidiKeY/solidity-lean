...
| Module | What it is |
|---|---|
| `Theory/Storage.lean` | `structRules.key` as a term algebra: `StValue`, `selectSt`, `find`, the delete family, and `save` as `write` (the walk, not upstream's symbol) with its leaf left as the one lazy constructor `merge` — KeY's `save(st, nil, v)`, read through by member sort so a struct written over a location keeps its mappings. Every taclet a theorem; `selectOnSaveCons` with no well-formedness hypothesis. `delValue` is member-wise through a leaf, as upstream's `delNode` reads are, and `selectSt_delValue` pushes a delete through a copy under `WellMerged` (the two sides agree in shape). The four `find`-over-`write` laws (`find_write_extends`/`_same`/`_prefix`/`_frame`) and their `save` forms — `Semantics` had only the first. Paths are `Seg`; `size` is `Seg.field "length"`. |
...
| `Theory/Denote.lean` | What a storage term means: `slotOf`/`putAt` (one step of `SVal.save`, split), `denoteSt`, and `denote_write` — the theory's walk on the pre-state tree *is* `SVal.save`, errors included. The leaf evaluated (`mergeKeepMaps`) is `selectSt`'s reading of it, mapping members the source omits included (`keptMaps`), and `Denotes` — the term denotes the value and keeps doing so under every selector — is closed under the copy leaf (`Denotes.merge_struct` for two struct sides that agree in kind, `merge_mtSt`, `merge_of_not_struct`), so `denote_find_of` reads through a copy. `denote_delAt`, and `denote_save`/`_prim`: upstream's `save`, leaf included, collapses to the write off a mapping-carrying location and for every non-struct payload. (There is no fresh-slot arm: the slot a `push` lands on is the one a `pop` cleared.) No type parameter and no well-formedness predicate; the reasons are in the docstring. |
| `Update/Theory.lean` | A rule's stated update read as a KeY term: `storageRhs_eq_theory`, `heapRhs_eq_theory`; `denoteMem`/`denoteMV` resolve a path identity against the heap, and `denoteMem_new` discharges KeY's freshness premise. This is what makes every taclet above a fact about the wp semantics rather than about a private model. Push/pop deviate from KeY's spelling, and every `save` is read as the walk under its leaf (`theorySave_eq_theoryWrite*`); the docstring says how and why. |
...
| Module | What it is |
|---|---|
| `Semantics.lean` | Run-time values and the executable state semantics; `SolidityJudgment.check`/`Holds`. Total, no `partial` — Lean checks termination. Follows solc where KeY was more liberal (`docs/solc-alignment.md`). `SVal.array` carries the slots a `pop` cleared and gave back beside the live elements, and `pushSlot` is upstream's `delAt` at the pushed slot: a mapping nested in a popped element survives into the next `push`. |
...
- `MappingSideConditions.lean` — the mapping-shaped side conditions of the write and the delete, each refuted if dropped: M1 `WellMerged` in `selectSt_delValue` (an ill-sorted leaf), M2 `svalHasMapping` in `denote_save` (the fold's own content), M3 `tyHasMapping` in the storage capture template on a merely pure path, M4 the same on `storageIndexReadUnfoldRightSndResult`, where the path *and* index are simple — an index is evaluated, and a storage-kind index variable reads the store. M3/M4 are what only the simple-*field* rule does not need.
