...
| `Update/SequentSyntax.lean` | The `seq!{ … }` surface notation. |
...
- `Examples/Derivations/Sequents.lean` — the calculus's own derivations, update beside the shrinking program. **In the default build.**
- `Examples/Derivations/WorkedExamples.lean` — one chain per construct; rule sequences computed by `steps!`, not written. Root of the `SolidityExamples` target, **not** in the default build (~30 min CPU); run `./scripts/check-examples.sh`.
