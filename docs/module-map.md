...
| `Kernel/Syntax.lean` | The typed syntax, indexed by contract and local context: `SPath`/`Loc`/`Val`/`Src`, `Stmt C Γ Γ'`, `Prog`. Storage slice so far. `OpLoc`, a compound assignment's target. |
