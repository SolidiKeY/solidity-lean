...
| `Theory/Denote.lean` | What a storage term means: `slotOf`/`putAt` (one step of `SVal.save`, split), `denoteSt`, and `denote_save` — the theory's `save` on the pre-state tree *is* `SVal.save`, errors included. `denote_find`, `denote_delAt`, and `denote_copyAt`: upstream's mapping-preserving copy collapses to the write off a mapping-carrying target. No type parameter and no well-formedness predicate; the reasons are in the docstring. |
