...
| Symbolic soundness | `RuleSoundness.lean` | A `<rule>_sound` theorem per unfold rule with a non-empty residual block, relating the original statement and residual block under the interpreter; the two call rules are stated relative to inlining, and three statements carry a documented `sorry` (see `AGENTS.md`). |
