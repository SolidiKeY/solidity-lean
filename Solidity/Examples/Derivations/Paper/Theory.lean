...
```
⇝  {mem := M₁, storage := S₁, v := find(store(storage, alice, copyMem(∅, M₁, carol)), alice·age)}
...
private abbrev memWrite (r : Nat) (a : Seg) (v : Int) : Memory :=
...
sol_rewrite deepFieldWriteValue (s : Struct) :
    find (save s [alice, account, balance] (StValue.int 10)) [alice, account, balance]
...
sol_rewrite deepFieldWriteFrame (s : Struct) :
    find (save s [alice, account, balance] (StValue.int 10)) [bob, age]
  =[.findOnSaveDifferent] find s [bob, age]
...
sol_rewrite deepFieldWritePrefix (s : Struct) :
    find (save s ([alice] ++ [account, balance]) (StValue.int 10)) [alice]
  =[.findOnSavePrefix]
    StValue.st (save (asStruct (find s [alice])) [account, balance] (StValue.int 10))
...
subsequent field read sees `find(S₃, tokens[0].value) = 7`".
...
sol_rewrite pushPopSlotValue
    ...
    find S3 [tokens, .at 0, value]
  =[.findOnSaveDifferent] find S2 [tokens, .at 0, value]
...
sol_rewrite memoryAliasIdentity (r : Nat) (ty : RefTy) :
...
sol_rewrite memoryFieldValue (r : Nat) :
...
`find`". -/
sol_rewrite storageToMemoryRootCopyValue (r : Nat) :
    XMemory.readIn [] (.copySt (.of .mtMem) r (stSave [alice, age] 34)) (.idC r [alice]) age
...
sol_rewrite storageToMemoryOtherRoot (r1 r2 : Nat) (m : XMemory) (s : Struct)
    (hne : r1 ≠ r2) :
    XMemory.readIn [] (.copySt m r1 s) (.idC r2 [alice]) age
  =[.readCopyStOther] XMemory.readIn [] m (.idC r2 [alice]) age
...
sol_rewrite memoryToStorageRootCopyValue (r : Nat) :
    XStruct.find [] (.storeSt (.of Struct.mtSt) alice
      (.st (.copyMem (memWrite r age 34) (.idC r [])))) [alice, age]
  =[.findPath]       XStruct.find [] (.copyMem (memWrite r age 34) (.idC r [])) [age]
  =[.findCopyMem]    XValue.ofMem (memWrite r age 34)
                       (Memory.readR [] (memWrite r age 34) (.idC r []) [age])
  =[.readRSingleton] XValue.ofMem (memWrite r age 34)
...
sol_rewrite memoryToStorageFromAliasValue (r : Nat) :
    XStruct.find [] (.storeSt (.of Struct.mtSt) alice
        (.st (.storeSt (.of Struct.mtSt) account
          (.st (.copyMem (memWrite r balance 10) (.idC r []))))))
      [alice, account, balance]
  =[.findPath]       XStruct.find [] (.storeSt (.of Struct.mtSt) account
                       (.st (.copyMem (memWrite r balance 10) (.idC r []))))
                       [account, balance]
  =[.findPath]       XStruct.find [] (.copyMem (memWrite r balance 10) (.idC r [])) [balance]
  =[.findCopyMem]    XValue.ofMem (memWrite r balance 10)
                       (Memory.readR [] (memWrite r balance 10) (.idC r []) [balance])
  =[.readRSingleton] XValue.ofMem (memWrite r balance 10)
                       (Memory.readIn [] (memWrite r balance 10) (.idC r []) balance)
  =[.readWriteEqual] XValue.prim (PrimVal.int 10)
...
sol_rewrite memoryToStorageNonsimplePathValue (r : Nat) :
    XStruct.find [] (.storeSt (.of Struct.mtSt) alice
        (.st (.storeSt (.of Struct.mtSt) account
          (.st (.storeSt (.of Struct.mtSt) token
  ...
  =[.findPath]       XStruct.find [] (.storeSt (.of Struct.mtSt) account
                       (.st (.storeSt (.of Struct.mtSt) token
                         (.st (.copyMem (memWrite r value 99) (.idC r []))))))
                       [account, token, value]
  =[.findPath]       XStruct.find [] (.storeSt (.of Struct.mtSt) token
                       (.st (.copyMem (memWrite r value 99) (.idC r [])))) [token, value]
  =[.findPath]       XStruct.find [] (.copyMem (memWrite r value 99) (.idC r [])) [value]
  =[.findCopyMem]    XValue.ofMem (memWrite r value 99)
                       (Memory.readR [] (memWrite r value 99) (.idC r []) [value])
  =[.readRSingleton] XValue.ofMem (memWrite r value 99)
                       (Memory.readIn [] (memWrite r value 99) (.idC r []) value)
  =[.readWriteEqual] XValue.prim (PrimVal.int 99)
